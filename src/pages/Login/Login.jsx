...
export function Login() {
  ...
  // Student Form State
  const [evalName, setEvalName] = useState('araviti lakshmi sai reshmitha');
  const [evalRollNo, setEvalRollNo] = useState('vtu26450');
  const [evalEmail, setEvalEmail] = useState('vtu26450@veltech.edu.in');
  const [evalStream, setEvalStream] = useState('07ca93b9-a58b-4dcd-af62-5bc03ba1f534'); // Maps to clientID (e.g. cse / uuid)
  const [evalAccessCode, setEvalAccessCode] = useState('juFphv');
  const [evalClientSecret, setEvalClientSecret] = useState('mRSrrQUFTsuMvqPr');
